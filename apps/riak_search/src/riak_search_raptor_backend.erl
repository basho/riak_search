...
%% receive_fold_catalog_results(CatRef, StreamRef, Folder, Acc, Pending, CatConn, StrConn) ->
%%     receive
%%         {catalog_query, _ReqId, "$end_of_results", _, _, _, _} ->
%%             io:format("catalog done\n"),
%%             %% 
%%             case FState#fstate.pending =:= [] and FState#fstate.str_ref =:= undefined of
%%                 true ->
%%                     {ok, Acc};
%%                 false ->
%%                     receive_fold_catalog_results(FState#fstate{cat_ref = undefined})
%%             end;
%%         {catalog_query, CatRef, Partition, Index, Field, Term, _JSONProps} ->
%%             io:format("catalog ~p/~p/~p/~p\n", [Partition, Index, Field, Term]),
%%             %% If not currently streaming, kick off a new stream
%%             case FState#fstate.str_ref of 
%%                 undefined -> 
...
%%                                         TermBin,
%%                                         to_binary(Partition)),
%%                     NewFState = FState#fstate{str_ref = StreamRef};                
%%                 _ ->
%%                     NewFState = FState#fstate{pending = [{Partition, Index, Field, Term} | 
%%                                                          FState#fsate.pending]}
%%             end,
%%             receive_fold_catalog_results(NewFState)


%%         {stream, StreamRef, timeout} ->
%%             %io:format("~p/~p/~p stream socket timeout\n", [IndexBin, FieldBin, TermBin]),
%%             error_logger:warning_msg("fold_stream_process: raptor socket timed out: ~p.~p.~p~n",
%%                                       [IndexBin, FieldBin, TermBin]),
%%             exit(Conn, kill),
%%             {error, socket_timeout};
%%         {stream, StreamRef, "$end_of_table", _, _} ->
%%             %io:format("~p/~p/~p stream end of table\n", [IndexBin, FieldBin, TermBin]),
%%             %% io:format("fold_stream_process: table complete: ~p.~p.~p~n",
%%             %%     [Index, Field, Term]),
%%             {ok, Acc};
%%         {stream, StreamRef, Value, Props, KeyClock}=_Msg2 ->
%%             %io:format("~p/~p/~p - ~p/~p/~p\n", [IndexBin, FieldBin, TermBin, Value, Props, KeyClock]),
%%             case Props of
%%                 <<>> ->
%%                     Props2 = [];
%%                 _ ->
...
%%             NewAcc = Folder(Key,Val,Acc),
%%             fold_stream_process(StreamRef, IndexBin, FieldBin, TermBin,
%%                                 Folder, NewAcc, Conn)%;

%%        %% Msg ->
%%        %%      io:format("unexpected catalog message\n"),
%%        %%      FoldResultPid ! {fold_result, error},
%%        %%      exit(Conn, kill),
%%        %%      throw({unexpected_msg, Msg})
%%     after ?FOLD_TIMEOUT ->
%%             %io:format("catalog timeout\n"),
%%             exit(Conn, kill),
%%             throw(fold_timeout)
%%     end.
...
%% old_fold_catalog_process(CatRef,
%%                      FoldResultPid,
%%                      CatalogDone,
%%                      StreamProcessCount,
...
%% fold_stream_process(StreamRef, IndexBin,
%%                     FieldBin, TermBin, Folder, Acc, Conn) ->
%%     receive
%%         {stream, StreamRef, timeout} ->
%%             %io:format("~p/~p/~p stream socket timeout\n", [IndexBin, FieldBin, TermBin]),
%%             error_logger:warning_msg("fold_stream_process: raptor socket timed out: ~p.~p.~p~n",
...
%%                                 Folder, NewAcc, Conn)%;
%%         %% Msg ->
%%         %%     io:format("~p/~p/~p unexpected msg ~p\n", [IndexBin, FieldBin, TermBin, Msg]),
%%         %%     {error, {unexpected_msg, Msg}}
%%     after ?FOLD_TIMEOUT ->
%%             %io:format("~p/~p/~p stream receive timeout\n", [IndexBin, FieldBin, TermBin]),
...
%% old_fold_stream_process(CatalogProcessPid, FoldResultPid, StreamRef, IndexBin,
