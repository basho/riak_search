...
test_inner({solr_select, Params, Validators}, _Root) ->
    ...
    QS1 = [F(K, V) || {K, V} <- Params],
    QS2 = string:join(QS1, "&"),

    %% Run the query...
    Port = app_helper:get_env(riak_core, web_port),
    Url = io_lib:format("http://127.0.0.1:~p/solr/test/select?~s", [Port, QS2]),
    inets:start(),
    try http:request(lists:flatten(Url)) of
        {ok, {{_, 200, _}, _, Body}} ->
            ...
            case validate_results(Length, Results, Validators) of
                pass -> 
                    io:format("~n    [√] PASS SOLR SELECT » ~s (~s)~n", [Query, QS2]);
                {fail, Errors} ->
                    io:format("~n    [ ] FAIL SOLR SELECT » ~s (~s)~n", [Query, QS2]),
                    [io:format("        - ~s~n", [X]) || X <- Errors]
            end;
        {ok, {{_, Status, _}, _, _}} ->
            io:format("~n    [ ] FAIL SOLR SELECT » ~s (~s)~n", [Query, QS2]),
