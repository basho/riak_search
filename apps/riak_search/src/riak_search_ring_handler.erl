...
-module(riak_search_ring_handler).
...
ensure_vnodes_started(Ring) ->
    ...
    VNodes2Start =
        case {length(AllMembers), hd(AllMembers) =:= node()} of
            ...
            _ ->
                {ok, Excl} = gen_server:call(riak_search_vnode_master, get_exclusions, 15000),
    ...
    case VNodes2Start of
        ...
        _ ->
            [begin
                 gen_server:cast({riak_search_vnode_master, node()}, {I, start_vnode})
             end|| I <- VNodes2Start]
    end.
