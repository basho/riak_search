...
%%     index_terms_batch(Pid, Terms1, Timeout, false).

%% index_terms_batch(_Pid, [], _, IsOverloaded) ->
%%     ok;
%% index_terms_batch(Pid, Terms, Timeout, IsOverloaded) ->
...
%%     %% Send the batch and loop.
%%     Status = gen_fsm:sync_send_event(Pid, {index, Batch}, Timeout),
%%     IsOverloaded1 = (IsOverloaded orelse Status /= ok),
%%     index_terms_batch(Pid, Rest, Timeout, IsOverloaded1).
...
%% add_terms(Terms, From, State) ->
%%     %% Append the new terms.
%%     NewTerms = State#state.terms ++ Terms,
%%     OverloadThreshold = State#state.overload_thresh,

%%     %% Check if we are overloaded.
%%     case length(NewTerms) < OverloadThreshold of
%%         true ->
%%             %% If not, return ok.
%%             NewState = State#state { terms=NewTerms },
%%             {ok, NewState};
%%         false ->
%%             %% If so, then add a waiter.
%%             NewWaiters = [From|State#state.overload_waiters],
%%             NewState = State#state { terms=NewTerms,  overload_waiters=NewWaiters},
%%             {overload, NewWaiters}
%%     case  
