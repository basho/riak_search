...
'''
   ...
   "favorites_book", and "favorites_album".  You could later query
   this data with queries like,
   "bio:enginer AND favorites_album:mystery".
...
'''
   ...
   Would cause four fields to be indexed: "person_name", "person_bio",
   "person_favorites_item", and "person_favorite_item@type".  The
   ...
   "person_bio:enginer AND person_favorites_item:mystery".
...
*** Other Data Encodings
    ...
    'rs_extractfun' to one of the following values:
    ...
    + {jsanon, {Bucket, Key}}, where Bucket and Key name a Riak object
      that contains the source of a Javascript function to call for
      extraction.
    ...
    + {jsfun, Name}, where Name is the name of a pre-defined
      Javascript function, stored in a file in your js_path, to call
      for extraction.
    ...
    + {struct, JsonProplist}, where JsonProplist is a list of
      2-tuples, as mochijson2 would product from decoding a JSON
      ...
      arguments.  The first argument will be a riak_object to index,
...
'''
    ...
    The modules riak_search_kv_json_extractor,
    riak_search_kv_xml_extractor, and riak_search_kv_raw_extractor
...
** Querying Integrated with Map/Reduce
   ...
   Riak KV data that has been indexed by Search can be queried using
   any of the interfaces already discussed (such as the Solr HTTP
   interface, and the search-cmd script), but is also available for
   use in map/reduce queries.

   The syntax is fairly simple.  In the "inputs" section of your
   map/reduce query, use the new "modfun" specification, naming
   "riak_search" as your module, "mapred_search" as your function, and
...
'''
   ...
   Here are a few examples of how to kick off a map/reduce query with
   a Search process using other clients:

   Erlang Protocol Buffers Client (the Erlang HTTP client looks the
   same, except for a different module name):

```
riak_pb_socket:mapred(
   Client,
   {modfun, riak_search, mapred_search,
    [<<"article">>, <<"text:seven">>]},
   [...normal mapreduce query phases...]).
'''

   TODO Python, Java, PHP, Javascript, Ruby
...
** Merge Index Settings
   ...
   + buffer_rollover_size :: Maximum size of the in-memory buffer
   ...
   + buffer_delayed_write_size :: Bytes to accumulate in the
   ...
   + segment_query_read_ahead_size :: Size of the file read-ahead
        buffer, in bytes, to use when looking up results in a query.

   + segment_compact_read_ahead_size :: Size of the file read-ahead
        buffer, in bytes, to use when reading a segment for compaction.

   + segment_file_buffer_size :: Amount of segment compaction data to
        batch, in bytes, before writing to the file handle. This should be less
        than or equal to segment_delayed_write_size, otherwise that
        setting will have no effect.

   + segment_delayed_write_size :: Size of the delayed write buffer in
        bytes. Once this is exceeded, the compaction buffer is flushed
        to disk.

   + segment_delayed_write_ms :: Interval at which data will be
   ...
   + segment_full_read_size :: Segment files below this size will be
        ...
        *max_compact_segments* directly affects the maximum amount of
        RAM that a compaction can take.

   + segment_block_size :: Determines the block size across which a
   ...
   + segment_values_staging_size :: Maximum number of values to hold
        in memory before compressing the batch and adding to the
        output buffer.

   + segment_values_compression_threshold :: Since compression is more
   ...
   + segment_values_compression_level :: zlib compression level to use
