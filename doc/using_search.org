...
*** Install BerkeleyDB
    ...
    If you are installing a pre-built Riak Search package, unzip the BDB
    archive and copy =lib/je-4.0.103.jar= to your Riak Search
    installation under =lib/raptor-VERSION/priv/raptor_server=.

    If you are installing Riak Search from source, unzip the BDB
    archive and copy =lib/je-4.0.103.jar= to your Riak Search
    directory, under =apps/raptor/java_src/raptor/lib/=.
...
*** Notes
    ...
    4. If you wish to keep the BDB native library in somewhere other
       ...
       option in start.sh to reflect the new location (wrapped for
       line-length, enter without spaces):

       : -Djava.library.path=/usr/local/lib:
       : java.library.path=/usr/local/lib:/usr/local/BerkeleyDB.4.8/lib

    5. Raptor has only been tested on Mac OS X and Linux.

    6. If you see this, upgrade to Java 1.6:

       : [javac] bad class file: ...snip.../db.jar(com/sleepycat/db/
       : Environment.class)
