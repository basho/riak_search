...
* Introduction
  ...
  Basho Search is a distributed index and full-text search engine
...
** Requirements
   ...
   + Ant
   + gcc toolchain
   + BDB 5.0.21 - (Installation instructions below.)

** Installation
   
*** Install BerkeleyDB
   
    Download the source code for v. 5.0.21:
    http://download.oracle.com/berkeley-db/db-5.0.21.NC.tar.gz

    If you are running on a Mac, set build flags and copy header
    ...
    Finally, extract, build, and install BerkeleyDB:
...
*** Unzip and install Riak Search*
    ...
    Change to the rel/riak directory
...
*** Start Riak
    ...
    From within the rel/riak directory, start Riak:
    
    : bin/riak console
    
    Or, use =bin/riak start= to start riak\_search in the background.


** Major Components
   
   Basho Search is comprised of:
   ...
   Note that even though Riak KV is included in Basho Search, it
...
* Schema
  ...
  Basho Search was designed to work seamlessly with Riak. As a
...
** Defining an Index Schema
   ...
   *NOTE:* Basho Search currently stores schema definition files in
   the lib/riak\_search/priv/ directory. This is covered in #289.

   *NOTE:* The commands below are not yet implemented. This is covered
   in #318.


   Schema definitions are stored in Basho Search under a =schema=
   ...
   : # Set an index schema.
   : bin/basho_search set_schema Index SchemaFile
   : 
   : # View the schema for an Index.
   : bin/basho_search view_schema Index
...
** Schema Properties
   ...
   + *version* - /Atom/, currently unused by Basho Search.
...
** Data Types
   ...
   Basho Search supports the following data types:
...
* Indexing
  ...
  There are numerous ways to index a document in Basho Search.

** Indexing via the Command Line

   *NOTE:* This command line tool does not yet exist. It is covered in
   issue #318.

   The easiest way to index documents stored on the filesystem is to
   use the =basho_search= command line tool:

   : bin/basho_search index <INDEX> <PATH>
   ...
   : bin/basho_search index my_index files/to/index/*.txt
...
** Deleting via the Command Line
   ...
   *NOTE:* This command line tool does not yet exist. It is covered in issue #318.

   To remove previously indexed files from the command line, use the
   =basho_search= command line tool:

   : bin/basho_search delete <INDEX> <PATH>
   ...
   : bin/basho_search delete my_index files/to/index/*.txt
...
** Indexing via the Erlang API
   ...
   *NOTE:* This function is not yet supported. It is covered in issue #320.
...
** Deleting via the Erlang API
   ...
   *NOTE:* This function is not yet supported. It is covered in issue #329.
...
** Indexing via the Solr Interface
   ...
   Basho Search supports a Solr-compatible interface for indexing
...
** Deleting via the Solr Interface
   ...
   *NOTE:* Search does not yet support deleting documents from a
   specific index via "index:docid" format. The index name must be
   specified in either the endpoint URL or query parameters. (See /Indexing via
   the Solr Interface/ section above.) This is covered in #330.
...
** Indexing via Riak Post-Commit Hook
   ...
   *NOTE:* Search is not yet integrated with Riak. This is scheduled
   ...
   Basho Riak (a distributed Key/Value store) can be configured to
   index new data automatically using Basho Search. Indexing is
   ...
   The following post-commit hook provides integration with Basho Search:
   
   : {"mod": "riak_search", "fun": "index"}

   Pending Questions:
   
   + How does Riak know /where/ Basho Search is located?
...
** Query Syntax
   ...
   Basho Search follows the same query syntax as Lucene, detailed
...
*** Range Searches
    ...
    brackets to specify an inclusive range, and brackes (mustaches)
    to specify an exclusive range.

    The following example will return documents with words containing "red" and
    "rum", plus any words in between.

    : "[red TO rum]" 
    
    The following example will return documents with words in between "red" and
    "rum":
...
*** Boosting a Term
    ...
    In the following example, documents with the term "red" will have their score
    boosted:
...
*** Grouping
    ...
    Clauses in a query can be grouped using parentheses. The following query
    returns documents that contain the terms "red" or "blue", but not "yellow":

    : (red OR blue) AND NOT yellow

** Querying via the Search Shell

   *NOTE:* For now, you need to start the shell from Erlang using
   =riak_search_shell:start()= or
   =riak_search_shell:start(Index)=. Starting via command line is
   covered in #318.

   The Search Shell is the easiest way to run interactive queries
   against Search. To start the shell, run:

   : bin/basho_search shell [INDEX]
...
** Querying via the Command Line
   ...
   : bin/basho_search search [INDEX] QUERY

   For example:

   : bin/basho_search search "books" "title:\"See spot run\""

   This will display a list of Document ID values matching the
   query. To conduct a document search, use the *search\_doc*
   command. For example:

   : bin/basho_search search_doc "books" "title:\"See spot run\""
...
** Querying via the Solr Interface
   ...
   Basho Search supports a Solr-compatible interface for searching
   ...
   + *q.op=OPERATION* - Allowed settings are either "and" or
     "or". Overrides the /default_op/ setting in the schema file. Default is "or".
   ...
   *NOTE:* Currently, all query output will be served in JSON. This is
   analogous to specifying =wt=json= on a regular Solr
   query. Outputting as XML is covered in #189.

** Faceted Queries via the Solr Interfae
   
   *NOTE:* The facet features listed below do not yet exist. These are covered in #326.
...
** Query Scoring
   ...
   The key difference is in how Basho Search calculates the Inverse
   Document Frequency. The equations described on the /Similarity/
   page require knowledge of the total number of documents in a
   collection. Basho Search does not maintain this information for a
