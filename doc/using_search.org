...
* Introduction
  ...
  Python, Ruby, and Erlang APIs) to perform queries that return a list
  of bucket/key pairs matching the query. Alternatively, you can use a
  ...
  determine required fields, the unique key, the default analyzer, and
  which analyzer should be used for each field. Field aliases
  ...
  provides higher overall query throughput with large data sets. (This
  can come at the expense of higher-latency queries for especially
  large *result* sets.)
