...
*** Install BerkeleyDB
    ...
    This archive contains a pre-built version of BDB Java Edition.

    If you are using a pre-built Riak Search package, unzip the BDB
    archive and copy =lib/je-4.0.103.jar= to your Riak Search
    installation under =lib/raptor/priv/=.

    If you are building Riak Search from source, unzip the BDB archive
    and copy =lib/je-4.0.103.jar= to your Riak Search directory, under
    =apps/raptor/java_src/raptor/lib/=.
...
** The Default Schema
   ...
   + *FIELDNAME\_facet* - Facet field. Uses Whitespace analyzer.
...
** Defining a Schema
   ...
   The schema definition for an index for an are stored in the Riak
   ...
   definitions, especially settings such as /facet/ or /type/, that
